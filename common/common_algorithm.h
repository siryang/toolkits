...
#define Between(a, b, c) ((b) <= (a) && (a) <= (c)) //a ¡Ê [b, c]
	...
	inline Iterator filter(Iterator first, Iterator last, InstanceType* instance, Filter filter, Procer proc)
	{
		for (; first != last; ++first)
		{
	...
	std::vector<std::wstring> split(const wchar_t* str, const wchar_t* limit);
	std::vector<std::string> split(const char* str,const char* limit);
